...
export function solveTwoBoneIK(arm, targetWorld, poleWorld) {
    ...
    _tempVec.sub(_targetDir.clone().multiplyScalar(poleDotTarget));
    ...
    if (_tempVec.lengthSq() < 0.0001) {
        ...
        _tempVec.sub(_targetDir.clone().multiplyScalar(_tempVec.dot(_targetDir)));
    }
    _tempVec.normalize();
    ...
    if (shoulder.parent) {
        shoulder.parent.getWorldQuaternion(parentQuatInverse);
        parentQuatInverse.invert();
    }

    // Convert toElbow to shoulder's local space
    const toElbowLocal = _toElbow.clone().applyQuaternion(parentQuatInverse);

    // Shoulder rotation: rotate from default (-Y) to point at elbow
    const defaultDir = new THREE.Vector3(0, -1, 0);
    shoulder.quaternion.setFromUnitVectors(defaultDir, toElbowLocal);
    ...
    shoulder.getWorldQuaternion(elbowWorldQuat);

    // Invert to get world-to-elbow-local transform
    const elbowWorldQuatInverse = elbowWorldQuat.clone().invert();

    // Convert elbow-to-wrist direction to elbow local space
    const toWristLocal = _elbowToWrist.clone().applyQuaternion(elbowWorldQuatInverse);

    // Elbow rotation: from default (-Y) to point at wrist
    elbow.quaternion.setFromUnitVectors(defaultDir, toWristLocal);
}
