...
export class BrainStream extends EventTarget {
     ...
     * @param {boolean} savePassphrase - Whether to save the passphrase
     * @returns {Promise<boolean>}
     */
    async authenticate(code, savePassphrase = true) {
        ...
        if (savePassphrase) {
            localStorage.setItem('brain_passphrase', code);
        }
        return true;
    }

    /**
     * Try to re-authenticate with stored passphrase
     * @returns {Promise<boolean>}
     */
    async tryReauthenticate() {
        const passphrase = localStorage.getItem('brain_passphrase');
        if (!passphrase) return false;
        try {
            await this.authenticate(passphrase, false);
            console.log('Re-authenticated with stored passphrase');
            return true;
        } catch (e) {
            console.log('Re-auth failed:', e.message);
            localStorage.removeItem('brain_passphrase');
            return false;
        }
    }
    ...
    connect() {
        ...
        this.ws.onclose = async (e) => {
            ...
            if (authErrorCodes.includes(e.code)) {
                ...
                console.log(`Auth error (code ${e.code}), trying re-authentication`);
                this.clearToken();
                ...
                if (this.reconnectAttempts >= this.maxReconnectAttempts) {
                    console.log(`Max reconnect attempts (${this.maxReconnectAttempts}) reached, trying re-auth`);
                    this.clearToken();
                    const reauthed = await this.tryReauthenticate();
                    if (reauthed) {
                        this.reconnectAttempts = 0;
...
function extractMessageFromCurl(cmd) {
    ...
    // Match -d followed by single or double quoted JSON, handling escaped quotes
    ...
    // Handle escaped quotes in the heredoc content
